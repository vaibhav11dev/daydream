...
class daydream_ThemeFrameworkMetaboxes {
	...
	public function daydream_textarea( $id, $label, $desc = '', $default = '' ) {
		...
		$html	 .= '</label>';
		$html	 .= '<div class="field">';
		$html	 .= '<textarea cols="120" rows="10" id="daydream_' . esc_attr($id) . '" name="daydream_' . $id . '">' . $value . '</textarea>';
		if ( $desc ) {
			$html .= '<p>' . esc_html($desc) . '</p>';
		}
		$html	 .= '</div>';
	...
	public function daydream_upload( $id, $label, $desc = '' ) {
		...
		if ( $daydream_upload_src ) :
			$html	 .= '<input type="text" class="upload_field" value="' . $daydream_upload_src . '" /></br>';
			if ( $id != 'webm' && $id != 'mp4' && $id != 'ogv' )
				$html	 .= '<img src="' . esc_url($daydream_upload_src) . '" class="redux-option-image" style="width:60px; height:60px;" />';
		endif;
		$html .= '</div>';

		$html	 .= '<input class="daydream_upload_button button ' . $hide1 . '" id="daydream-media-upload-' . esc_attr($id) . '" data-media-id="' . esc_attr($id) . '" type="button" value="Upload" />';
		$html	 .= '<input class="daydream_remove_button button ' . $hide2 . '" id="daydream-media-remove-' . esc_attr($id) . '" data-media-id="' . esc_attr($id) . '" type="button" value="Remove" />';
		if ( $desc ) {
			$html .= '<p>' . esc_html($desc) . '</p>';
		}
		$html	 .= '</div>';
	...
	public function daydream_render_option_tabs( $requested_tabs, $post_type = 'default' ) {
		...
		foreach ( $requested_tabs as $key => $tab_name ) {
			...
			printf( '<div class="dd_metabox_tab %s" id="dd_tab_%s">', $class_active, $tab_name );
