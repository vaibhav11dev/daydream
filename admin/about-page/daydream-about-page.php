...
class Daydream_About_Page {
	 ...
	 *
	 * @var Daydream_About_Page $instance The Daydream_About_Page instance.
	 */
	private static $instance;

	/**
	 * The Main Daydream_About_Page instance.
	 *
	 * We make sure that only one instance of Daydream_About_Page exists in the memory at one time.
	 *
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'dd_about_plugin_info_' . $slug );
	...
	/**
	 * Check if a slug is from cf7, yith returns the correct slug for them.
	 *
	 * @param string $slug Plugin slug.
	 *
	 * @return string
			...
			if ( ! empty( $child_themes['content'] ) && is_array( $child_themes['content'] ) ) {
				...
				for ( $i = 0; $i < count( $child_themes['content'] ); $i ++ ) {
					...
					if ( ! empty( $child['image'] ) ) {
						...
						echo '</div><!--dd-about-page-child-theme-image-->';
						echo '</div><!--dd-about-child-theme-->';
					}// End if().
				}// End for().
				echo '</div>';
			}// End if().
			...
			if ( get_option( $this->theme_slug . '_required_actions' ) ) {
				...
				switch ( esc_html( $_GET['todo'] ) ) {
					case 'add':
						...
						break;
					case 'dismiss':
						$dd_about_page_show_required_actions[ absint( $action_id ) ] = false;
						break;
				}

				update_option( $this->theme_slug . '_required_actions', $dd_about_page_show_required_actions );
